...
type Props = {
  loading?: boolean;
  /** A single sponsorship purchase entry (usually current) */
  entry?: SponsorshipApiRow | null;
  /** parent can open PKImageDialog */
  onViewSlip?: () => void;
};
...
export default function DesktopSponsorshipPaymentDetailsSection({
  loading,
  entry,
  onViewSlip,
}: Props) {
  ...
  return (
    <Box id='payment-details' sx={{ mt: 6 }}>
      ...
      <Box
        sx={{
          display: 'grid',
          gridTemplateColumns: { xs: '1fr', md: '1fr 320px' }, // left | right slip
          gap: 4,
          alignItems: 'start',
        }}
      >
        ...
        <Box>
          ...
          >
            <BankBlock title='Paid into' bank={entry?.payment_into ?? null} />
            <BankBlock title='Paid from' bank={entry?.payment_from ?? null} />
          </Box>
