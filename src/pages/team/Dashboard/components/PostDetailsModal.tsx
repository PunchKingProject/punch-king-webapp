...
import type { TeamPost } from '../../pages/team/Dashboard/api/dashboard.types.ts'; // Adjust path if needed
...
export default function PostDetailsModal({ post, open, onClose, onSuccess }: Props) {
  ...
  const handleUpdate = async () => {
    setLoading(true);
    try {
      // NOTE: Replace this with your actual customFetch or axios instance
      // e.g., await customFetch.patch(`/post/`, { post_id: post.id, ...formData })
      
      const token = localStorage.getItem('token'); // Fallback if not using an interceptor
      ...
      onSuccess(); // Triggers the dashboard to close the modal and optionally refetch
    } catch (error: any) {
